...
# -------------------------
# RUTA EXTRA PARA ARREGLAR 404 SIN CAMBIAR TUS PLANTILLAS
# -------------------------
@app.route("/exportar_csv_con_datos_cifrados/")
def exportar_csv_auto():
    """
    Esta ruta permite exportar CSV sin enviar oferta_id.
    Detecta automáticamente el ID desde la URL anterior.
    """
    ref = request.referrer or ""
    m = re.search(r"/postulantes/([0-9a-fA-F]{24})/?", ref)
    if not m:
        abort(404)

    oferta_id = m.group(1)
    return redirect(url_for("exportar_csv_con_datos_cifrados", oferta_id=oferta_id))
