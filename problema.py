...
def calculate_age(born: date):
    ...
    return today.year - born.year - (
        (today.month, today.day) < (born.month, today.day)
    )

STOPWORDS = {
    "de", "la", "el", "y", "en", "a", "los", "las",
    "con", "para", "por", "un", "una", "es", "se",
    "su", "que", "al", "del"
}
...
def eliminar_oferta(oferta_id):
    ...
    flash(
        "Oferta eliminada" if ok else f"Error: {err}",
...
def postular(oferta_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            flash(
                "Error guardando la postulación. Intenta de nuevo.",
                "danger",
            )
...
def ver_postulantes(oferta_id):
    ...
    postulantes_cursor = postulantes_col.find(
        {"oferta_id": oid}
    ).sort("_created", -1)
